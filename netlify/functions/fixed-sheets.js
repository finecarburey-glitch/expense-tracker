// Fixed Google Sheets integration with robust private key handling

let sheetsService = null;

// Try to initialize Google Sheets service with multiple private key format attempts
try {
  ...
  if (sheetId && clientEmail && privateKey) {
    ...
    // Method 2: If that doesn't work, try replacing with actual newlines from the JSON
    if (!formattedPrivateKey.includes('-----BEGIN PRIVATE KEY-----')) {
      // Try to reconstruct the key from the JSON format
      const keyLines = privateKey.split('\\n');
      formattedPrivateKey = keyLines.join('\n');
    }
    
    // Method 3: If still not working, try the original format
    if (!formattedPrivateKey.includes('-----BEGIN PRIVATE KEY-----')) {
      formattedPrivateKey = privateKey;
    }
    
    console.log('Formatted private key starts with:', formattedPrivateKey.substring(0, 50));
    console.log('Formatted private key ends with:', formattedPrivateKey.substring(formattedPrivateKey.length - 50));
