...
export class GoogleSheetsService {
  ...
  constructor() {
    ...
    this.sheetId = functions.config().google.sheets_id;
    
    // Initialize auth
    this.auth = new google.auth.GoogleAuth({
      credentials: {
        client_email: functions.config().google.service_account_email,
  ...
  async getMonthlyReport(month: number, year: number) {
    try {
      ...
      const prevMonthTotal = prevMonthExpenses.reduce((sum: number, exp: any) => sum + exp.amount, 0);
      ...
      return {
        month, year, totalSpent, categoryTotals, changeFromPrevMonth,
        percentChange, expenseCount: expenses.length, prevMonthTotal
      };
    } catch (error) {
  ...
  // Reclassify expenses
  async reclassifyExpenses(oldCategory: string, newCategory: string) {
    try {
      const expenses = await this.getExpenses();
      const expensesToUpdate = expenses.filter((exp: any) => exp.category === oldCategory);
      
      if (expensesToUpdate.length === 0) {
        return { success: true, updatedCount: 0 };
      }

      // Update each expense (simplified for Firebase)
      // In a real implementation, you'd update the Google Sheets rows
